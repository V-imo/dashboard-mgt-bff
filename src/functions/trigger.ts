...
import { EntityParser, item } from "dynamodb-toolbox";
import {
...
import { RoomElementEntity } from "../core/room-element/room-element.entity";
import { RoomEntity } from "../core/room/room.entity";
...
export const handler = async (event: DynamoDBStreamEvent) => {
  await Promise.all(
    event.Records.map(async (record: DynamoDBStreamRecord) => {
      ...
      } else if (object._et.S === RoomEntity.entityName) {
        const { item } = RoomEntity.build(EntityParser).parse(
          unmarshall(object)
        );
        if (record.eventName === "INSERT") {
          await eventBridge.send(RoomCreatedEvent.build(item));
        } else if (record.eventName === "REMOVE") {
          await eventBridge.send(RoomDeletedEvent.build(item));
        } else if (record.eventName === "MODIFY") {
          await eventBridge.send(RoomUpdatedEvent.build(item));
        }
      } else if (object._et.S === RoomElementEntity.entityName) {
        const { item } = RoomElementEntity.build(EntityParser).parse(
          unmarshall(object)
        );
        if (record.eventName === "INSERT") {
          await eventBridge.send(RoomElementCreatedEvent.build(item));
        } else if (record.eventName === "REMOVE") {
          await eventBridge.send(RoomElementDeletedEvent.build(item));
        } else if (record.eventName === "MODIFY") {
          await eventBridge.send(RoomElementUpdatedEvent.build(item));
        }
      }
    })
  );
